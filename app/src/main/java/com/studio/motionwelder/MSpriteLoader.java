...
//		DataInputStream dstrm = new DataInputStream(new String().getClass().getResourceAsStream("/reses"+spriteName));
		System.out.println("spriteName="+spriteName);
		DataInputStream dstrm = new DataInputStream(MainMidlet.getResourceAsStream(spriteName));
		try{
			dstrm.readShort(); 
			dstrm.readUTF();
