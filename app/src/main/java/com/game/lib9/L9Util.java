...
//////////////////////////////////////////有关字节数组的一些操作//////////////////////////////////////////////////////////////////////////////////
    ...
    public static void drawBut(Graphics g,Image butImg,int posX,int posY,Image selImg,boolean isFlag){
		int butW = butImg.getWidth();
		int butH = butImg.getHeight()/2;
		if(isFlag){
			g.drawRegion(butImg, 0, 0, butW, butH, 0, posX, posY, Graphics.VCENTER|Graphics.HCENTER);
			L9Util.drawSelIcon(g, selImg, butW, butH, posX, posY, MainMidlet.engine.frameCount);
		}else{
			g.drawRegion(butImg, 0, butH, butW, butH, 0, posX, posY, Graphics.VCENTER|Graphics.HCENTER);
		}
	}
	 ...
	 * @param y
	 */
	public static void drawSelIcon(Graphics g,Image image,
			int w,int h,int x,int y,int currentFrame){
		...
		posX[1] = x+w/2;
		posY[1] = y+h/2;
		g.drawRegion(image, 0, 0,tileW , tileH, 0, posX[0], posY[0], Graphics.LEFT|Graphics.TOP);//左上角
		g.drawRegion(image, tileW, 0,tileW , tileH, 0, posX[1], posY[0], Graphics.RIGHT|Graphics.TOP);//右上角
		g.drawRegion(image, 0, tileH,tileW , tileH, 0, posX[0], posY[1], Graphics.LEFT|Graphics.BOTTOM);
		g.drawRegion(image, tileW, tileH,tileW , tileH, 0, posX[1], posY[1], Graphics.RIGHT|Graphics.BOTTOM);
	}
}
