...
public class L9DialogMsg extends L9DialogBackground implements L9IState {
	public L9DialogMsg(Engine lib9) {
		super(lib9.FG);
		this.lib9 = lib9;
	}
	...
	public void Paint() {
		...
		lib9.FG.fillRoundRect(btnX + 1, btnY, btnW - 2, L9Config.FONT_H + 2,
				10, 10);
		lib9.FG.setColor(dialogBtnBgBorderColor);
		lib9.FG.drawRoundRect(btnX + 1, btnY, btnW - 2, L9Config.FONT_H + 2,
				10, 10);
		lib9.FG.setColor(dialogLineColor);
		for (int i = 0; strArr != null && i < strArr.length; i++) {
			// 绘制对话框内容，最后两行“是”与“否”居中绘制
			int YY = dialogY + dialogTopH + i
					* (L9Config.FONT_H + dialogLineSpace) + 2;
			int align = l9Str.K_Line_Align_Left;
			if (i >= strArr.length - 1) {
				align = l9Str.K_Line_Align_Center;
				l9Str.drawLine(lib9.FG, strArr[i], btnX, YY, btnW, align);
			} else {
				l9Str.drawLine(lib9.FG, strArr[i], dialogX
						+ dialogLineTextOffsetX, YY, dialogW, align);
			}
		}
	}
	...
	public void Update() {
		...
		if (lib9.getPointerX() > 0) {
			if (lib9
					.isInRect(lib9.getPointerX(), lib9.getPointerY(),
							new L9Rect(btnX, btnY, btnX + btnW, btnY
									+ L9Config.FONT_H))) {
