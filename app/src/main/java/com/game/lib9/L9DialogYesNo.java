...
public class L9DialogYesNo extends L9DialogBackground implements L9IState {
	...
	public void setYesNoDialog(String msg) {
		setYesNoDialog(L9Config.yesNoDialogTitle, msg,
				L9Config.yesNoDialogYesBtnText, L9Config.yesNoDialogNoBtnText,
				L9Config.yesNoDialogW);
	}

	public L9DialogYesNo(Engine lib9) {
		super(lib9.FG);
		this.lib9 = lib9;
	}
	...
	public void Paint() {
		...
		int YY = dialogY + dialogTopH + (strArr.length - 2)
				* (L9Config.FONT_H + dialogLineSpace);
		...
		lib9.FG.fillRoundRect(dialogX + 1, YY, dialogW - 2,
				L9Config.FONT_H + 2, 10, 10);
		...
		for (int i = 0; strArr != null && i < strArr.length; i++) {
			...
			if (i >= strArr.length - 2) {
				l9Str.drawLine(lib9.FG, strArr[i], dialogX, YY, dialogW,
						l9Str.K_Line_Align_Center);
			} else {
				l9Str.drawLine(lib9.FG, strArr[i], dialogX
						+ dialogLineTextOffsetX, YY, dialogW,
						l9Str.K_Line_Align_Left);
			}
		}
	}
	...
	public void Update() {
		if (lib9.isDragPointer()) {
			if (_msgDragBeginX == -1) {
				if (lib9.isInRect(lib9.getDragPointerX(), lib9
						.getDragPointerY(), new L9Rect(dialogX, dialogY,
						dialogX + dialogW, dialogY + dialogH))) {
		...
		if(!Config.Area.equals("SBY")) {
			if (lib9.isKeyPressed(Engine.K_KEY_UP | Engine.K_KEY_NUM2
					| Engine.K_KEY_DOWN | Engine.K_KEY_NUM8)) {
				_bYesNo = !_bYesNo;
			}
			if (lib9.isKeyPressed(Engine.K_KEY_FIRE | Engine.K_KEY_NUM5)) {
				lib9.resetKey();
				lib9.changeState(lib9.popState());
				lib9.setGlobalGraphics(false);
			}
		}else{
			if (lib9.isKeyReleased(Engine.K_KEY_UP | Engine.K_KEY_NUM2
					| Engine.K_KEY_DOWN | Engine.K_KEY_NUM8)) {
				_bYesNo = !_bYesNo;
			}
			if (lib9.isKeyReleased(Engine.K_KEY_FIRE | Engine.K_KEY_NUM5)) {
				lib9.resetKey();
				lib9.changeState(lib9.popState());
				lib9.setGlobalGraphics(false);
			}
		...
		if (lib9.getPointerX() > 0) {
			if (lib9.isInRect(lib9.getPointerX(), lib9.getPointerY(),
					new L9Rect(dialogX, btnY, dialogX + dialogW, btnY
							+ L9Config.FONT_H))) {
				if (!_bYesNo) {
					_bYesNo = true;
				} else {
					lib9.logicKeyPressed(lib9
							.getKeyCodeByLogicKey(Engine.K_KEY_FIRE
									| Engine.K_KEY_NUM5));
				}
			} else if (lib9.isInRect(lib9.getPointerX(), lib9.getPointerY(),
					new L9Rect(dialogX, btnY + L9Config.FONT_H, dialogX
							+ dialogW, btnY + 2 * L9Config.FONT_H))) {
				if (_bYesNo) {
					_bYesNo = false;
				} else {
					lib9.logicKeyPressed(lib9
