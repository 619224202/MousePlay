...
public class SortedArray {
	...
	public static void main(String [] args) {
		...
		String s1 = "commoney";
		System.out.println(s1.compareTo("common"));
		Object dummyobject = "dummy";
		SortedArray arr_accum = new SortedArray(20);
		arr_accum.put("key_"+0+".0", new Integer(0));
		...
		for(int i = 0;i<100;i++){
			...
			int arrSize = arr_accum.size;
			System.out.println("arrSize = "+arrSize);
			boolean bInsert = false;
			int index = 0;
			arr_accum.put("", new Integer(k));
			System.out.println(arr_accum);
		}
	}
	
	public void clearL9Obj(){
		for (int i=0; i<size; i++) {
			keys[i]=null;
			((L9Object)values[i]).removeDone();
			values[i]=null;
		}
		size=0; 
	}
